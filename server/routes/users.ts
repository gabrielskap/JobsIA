...
router.put('/:id', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const { rows: usersFound } = await pool.query('SELECT * FROM users WHERE id = $1', [id]);
    ...
    if (email && emailLower !== currentUser.email) {
      const existing = await pool.query('SELECT id FROM users WHERE email = $1 AND id <> $2', [emailLower, id]);
      if (existing.rows.length > 0) {
