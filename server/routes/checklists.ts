...
router.get('/:id/pdf', async (req: AuthRequest, res) => {
  try {
    ...
    const payload = {
      ...
      origem_destino: getField(['origem_destino', 'Caminho Físico Origem/Destino']),
      servidores: servidoresVal,
      servidor_origem: getField(['servidor_origem', 'Servidor de Origem', 'Servidor Origem']),
      servidor_destino: getField(['servidor_destino', 'Servidor de Destino', 'Servidor Destino']),
      diretorio_origem: getField(['diretorio_origem', 'Diretório Origem', 'Caminho Origem', 'Path Origem']),
      diretorio_destino: getField(['diretorio_destino', 'Diretório Destino', 'Caminho Destino', 'Path Destino']),
      capacidade_armazenamento: getField(['capacidade_armazenamento', 'capacidade', 'volume_disco', 'Quota / Armazenamento', 'Capacidade Estimada']),
      permissoes_usuario: getField(['permissoes_usuario', 'permissoes', 'usuario_proprietario', 'Permissões / Usuário Processo']),
